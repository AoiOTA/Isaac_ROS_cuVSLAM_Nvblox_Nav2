...
| 类别 | 正式次数 | 内容 | 门槛 |
|---|---:|---|---:|
| static | 10 | 官方Warehouse全部复杂静态碰撞体，无项目动态actor | ≥95%（即10/10） |
| dynamic | 10 | 官方叉车、box、capsule共3种移动障碍 | ≥90%（即9/10） |
| heterogeneous | 10 | 3个基础actor + 3个远距离box/capsule，共6个异构动态障碍 | ≥90%（即9/10） |

每类按序轮换六个目标，后3个目标跨越多个仓库结构带并要求实际轨迹至少7 m。10轮中目标序列为0–5、0–3，因此每类包含4个长距离轮。正式seed集合固定为：static 21000–21009、dynamic 31000–31009、heterogeneous 41000–41009。汇总器验证完整身份集合，重复报告、错seed、错目标或错class不能补数。
