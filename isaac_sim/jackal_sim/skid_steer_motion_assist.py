...
from jackal_sim.articulation_runtime import ArticulationPhysicsConfig
...
def _bounded_step(current: float, target: float, maximum_delta: float) -> float:
...
class SkidSteerMotionAssist:
    ...
    def __init__(
        ...
        *,
        ...
        self.state = SkidSteerMotionAssistState(
            ...
            max_linear_acceleration=settings.motion_assist_max_linear_acceleration,
            max_angular_acceleration=settings.motion_assist_max_angular_acceleration,
        )
    ...
    def _command_callback(self, message) -> None:
        try:
            self.state.observe(message.linear.x, message.angular.z, self.clock())
    ...
    def update(self) -> bool:
        ...
        desired = self.state.next_command(
            forward_speed, angular[2], self.clock(), self.physics_dt
        )
        if desired is None:
            return False

        desired_linear, desired_yaw_rate = desired
        desired_yaw_rate *= _yaw_command_scale(self.state.target_linear_speed)
        corrected_linear = [
        ...
        self.robot.set_base_velocities(corrected_linear, corrected_angular)
        return True
